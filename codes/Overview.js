...
{title:"Earnings",value:"$540000"},
{title:"Tasks Due",value:5},
];

const earningsData=[
  {month:"Jan",earnings:760},
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
  {summaryData.map((item,idx)=> (
    <div key={idx} className="bg-[#1e293b] text-white p-6 rounded-xl shadow-lg flex flex-col justify-between hover:scale-105 transform transition">
  <h2 className="text-gray-400 font-semibold">{item.title}</h2>
  <p className="text-2x-l font-bold mt-2">{item.value}</p>
</div>
))}
     </div>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
  ...
  <ResponsiveContainer width="100%" height={250}>
  <PieChart>
   <Pie data = {taskData} dataKey="value" nameKey="name"
...
<div className="bg-[#1e293b] p-6 rounded-xl shadow-lg">
  <h2 className="text-xl font-bold text-[#14b8a6]">Recent Activity</h2>
